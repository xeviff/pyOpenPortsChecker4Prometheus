...
def get_host_and_ports():
    global HOST, PORT
    HOST = str(
        input("Give me the IP address or DOMAIN NAME to scan ex(server.com): "))
    PORT = input(
        "Give me the PORT to scan or range using this format 80-8080: ")
...
def parse_scan_data():
    global is_port_range, HOST, PORT
    is_port_range = False
...
def scan_ports():
    global start_port, is_port_range, HOST, PORT, spoofed_payload
    print("\n\nStarting port scan against: " + HOST + "\n")
    while (start_port <= end_port):
        ...
        if is_port_range:
            spoofed_payload = {"port": str(start_port), "HOST": HOST}

        # r = requests.post(URL, headers=spoofed_headers, data=spoofed_payload)
        r = httpx.post(URL, headers=spoofed_headers,
                       data=spoofed_payload, timeout=None)

        if "Success:" in str(r.content):
            print("Port " + str(start_port) + " opened.")
        # else:
        #     print("Port " + str(start_port) + " closed.")
